...
public class CookieValuesRepositoryJdbcImpl implements CookieValuesRepository {
    ...
    //language=SQL
    private static final String SQL_FIND_ONE_BY_VALUE = "select c.value, su.* from cookie c join service_user su on c.user_id = su.id where c.value = ?";
    ...
    public Optional<CookieValue> findOneByValue(String value) {
        try {
            PreparedStatement statement = connection.prepareStatement(SQL_FIND_ONE_BY_VALUE);
            statement.setString(1, value);
            ResultSet resultSet = statement.executeQuery();

            if (resultSet.next()) return Optional.of(cookieValueRowMapper.mapRow(resultSet));
